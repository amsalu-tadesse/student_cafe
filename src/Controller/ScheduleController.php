...
class ScheduleController extends AbstractController
{
    ...
    /**
     * @Route("/update/ajax", name="schedule_update_ajax", methods={"GET","POST"})
     */
    public function scheduleUpdate(Request $request, ScheduleRepository $scheduleRepository): Response
    {
       ...
       if($schedule)
       {
        ...
        $nextSchedule = $scheduleRepository->findOneBy(['date'=> $schedule->getDate(), 'status'=>0],['id'=>'ASC']);
        
        $schedule->setStatus(2);//closed it
        if($nextSchedule)
        {
            $nextSchedule->setStatus(1);//enabled
        }
        ...
        return $this->json(
            [
                'success' => true,
                'nextSchedule' => $nextSchedule? $nextSchedule->getId():0
            ]
        );
       }
